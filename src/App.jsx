...
function Shell({ children, theme = "", isHome = false, isSetup = false }) {
  ...
  return <main className={`app-shell${appThemeClass}`}><BossBattleStyles /><section className={`phone-frame${frameThemeClass}${homeFrameClass}${setupFrameClass}`}><div className="blob blob-one" /><div className="blob blob-two" /><div className="content">{children}</div></section></main>;
}
...
export default function App() {
  ...
  if (screen === "grade") {
    return <Shell theme="normal" isSetup><div className="hero"><div className="icon-box icon-blue"><Zap /></div><h1>Normal</h1><p>Velg regneart.</p></div><div className="card input-card"><ModeButtons selectedMode={gameMode} includeMixed onSelect={(mode) => { setGameMode(mode); if (isTimeChallengeMode(mode)) setGameQuestionCount(10); setScreen("start"); }} /></div><Button variant="light" onClick={() => setScreen("home")} className="full top-space">Tilbake</Button></Shell>;
  }

  if (screen === "school") {
    return <Shell theme="school" isSetup><div className="hero"><div className="icon-box icon-blue"><Trophy /></div><h1>Skolekampen</h1><p>Velg skole.</p></div><div className="card input-card">{SCHOOL_OPTIONS.map((school) => <Button key={school} variant={schoolBattleSchool === school ? "primary" : "light"} onClick={() => { setSchoolBattleSchool(school); setScreen("schoolClass"); }} className="full top-space">{school}</Button>)}</div><Button variant="light" onClick={() => setScreen("home")} className="full top-space">Tilbake</Button></Shell>;
  }

  if (screen === "schoolClass") {
    return <Shell theme="school" isSetup><div className="hero"><div className="icon-box icon-blue"><Trophy /></div><h1>Skolekampen</h1><p>{schoolBattleSchool}</p><p className="small-note">Velg klasse.</p></div><div className="card input-card">{SCHOOL_BATTLE_GRADE_OPTIONS.map((grade) => <Button key={grade} variant={schoolBattleGradeLevel === grade ? "primary" : "light"} onClick={() => { setSchoolBattleGradeLevel(grade); setSchoolBattleGradeGroup(getSchoolBattleGradeGroup(grade)); setScreen("schoolMode"); }} className="full top-space">{getSchoolBattleClassLabel(grade)}</Button>)}</div><Button variant="light" onClick={() => setScreen("school")} className="full top-space">Tilbake</Button></Shell>;
  }
  ...
  if (screen === "start") {
    ...
    return (
      <Shell theme={startTheme} isSetup>
        <div className="hero">
