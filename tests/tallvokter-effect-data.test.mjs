...
test('de største runtimekartene holder den optimaliserte størrelsesgrensen', async () => {
  const mapsDir = new URL('../public/regnemester/maps/', import.meta.url);
  const limits = new Map([
    ['tallvokterens-rike-v4.webp', 3_600_000],
    ['world-map-v2.webp', 800_000]
  ]);
  ...
  for (const [filename, maximumBytes] of limits) {
    ...
    assert.ok(
      ...
      `${filename} er ${file.size} byte og overskrider runtimegrensen`
    );
  }
});
